...
def ekstrak():
    ...
    print('Done berhasil ekstrak database')
    cur.close()
...
def model_v2(kernel):
    ...
    modelWV.wv.save_word2vec_format('D:\BAHAN SKRIPSI\program_skripsi\models\saved_word2vec_v2.txt',binary=False)
    ...
    f = open('D:\BAHAN SKRIPSI\program_skripsi\models\saved_word2vec_v2.txt')
    for line in f:
        values = line.split()
        word = values[0]
    ...
    model.add(Conv1D(FILTERS_SIZE, KERNEL_SIZE, strides=1, activation='relu'))
    ...
    data.to_csv('D:\BAHAN SKRIPSI\program_skripsi\data\hasil_prediksi_v2.csv', index=False, header=False, sep="\t")
    ...
    with open('D:\BAHAN SKRIPSI\program_skripsi\models\Saved_Tokenize_v2.pickle', 'wb') as handle:
        pickle.dump(tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)

    model_json = model.to_json()
    with open('D:\BAHAN SKRIPSI\program_skripsi\models\saved_model_sentimen_v2.json','w') as json_file:
        json_file.write(model_json)

    model.save_weights("D:\BAHAN SKRIPSI\program_skripsi\models\saved_model_sentimen_v2.h5")
    ...
    print("BERHASIL SAVE HASIL PENGUJIAN VERSI 2 KE DATABASE!!!")
